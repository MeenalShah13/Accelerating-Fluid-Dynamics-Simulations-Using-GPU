...
if(barrier_type == "1" or barrier_type.lower() == "linear"):
	...
	for y in range(height):
		for x in range(width):
			# Ellipse equation: ((x - center_x) / horizontal_radius)^2 + ((y - center_y) / vertical_radius)^2 <= 1
...
startTime = time.time()
def nextFrame(arg):
	global startTime, animate
	if performanceData and (arg%100 == 0) and (arg > 0):
		endTime = time.time()
		print("Frame", arg, ":", "%1.1f" % (100/(endTime-startTime)), 'frames per second')
		startTime = endTime
	if arg == 2500:
